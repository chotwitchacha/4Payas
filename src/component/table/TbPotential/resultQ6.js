...
const ResultSix= ({data}) => {
    ...
    const concat6 = vertName1.concat(vertName2);
    ...
    const arrayToHTMLTableF = (myArray) => {
    ...
    $(function () {
        var output = getPivotArray(concat6, 0, 1, 2, 3);
        $("#pivotTable6").html(arrayToHTMLTableF(output)).css({"background-color": "#F5F5F5"},{ 'font-family':'sans-serif'});
    });
        ...
        <>
        {
                datas.length !== 0 ? (
                  ...
                  <CCardText style={{ lineHeight: 2, fontSize: 18, fontFamily: 'Prompt', textAlign: 'left', marginBottom: '20px' }}>
                      6. ความรับผิดชอบ : รับผิดชอบในหน้าที่ของตนเอง งานที่ได้รับมอบหมาย และรับผิดชอบต่อผู้อื่น ต่อส่วนรวม
                  </CCardText> 
                    <div style={{marginBottom: '20px'}} id="pivotTable6"></div>
                    </>
                ) : null
            }
