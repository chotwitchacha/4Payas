...
const ResultSeven= ({data}) => {
    ...
    const concat7 = vertName1.concat(vertName2);
    ...
    const arrayToHTMLTableG = (myArray) => {
    ...
    $(function () {
        var output = getPivotArray(concat7, 0, 1, 2, 3);
        $("#pivotTable7").html(arrayToHTMLTableG(output)).css({"background-color": "#F5F5F5"},{ 'font-family':'sans-serif'});
    });
        ...
        <>
        {
                datas.length !== 0 ? (
                  ...
                  <CCardText style={{ lineHeight: 2, fontSize: 18, fontFamily: 'Prompt', textAlign: 'left', marginBottom: '20px' }}>
                    7. มนุษยสัมพันธ์ และการอยู่ร่วมกัน : มีน้ำใจ เอื้อเฟื้อ และแบ่งปัน ต่อเพื่อนร่วมงาน และผู้อื่น
                  </CCardText> 
                    <div style={{marginBottom: '20px'}} id="pivotTable7"></div>
                    </>
                ) : null
            }
